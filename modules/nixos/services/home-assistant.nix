          ...
          sensor = [
            {
              ...
              state = ''
                ...
                {% elif remaining_mins >= 60 %}
                  {{ "%d hour%s %d minute%s left %s" % (remaining_mins//60, ('s',''')[remaining_mins//60==1], remaining_mins%60, ('s',''')[remaining_mins%60==1], message) }}
                {% else %}
                  {{ "%d minute%s left %s" % (remaining_mins, ('s',''')[remaining_mins==1], message) }}
                {% endif %}
              '';
            }
            {
